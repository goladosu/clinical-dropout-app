...
Original file is located at
    https://colab.research.google.com/drive/1EvXZkIkxA4ZO67rIX0ErTzrYVmJdXlES
"""
...
class ClinicalConsistencyTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, visit1_cols=None, visit2_cols=None, age_col="age", min_age=18):
...
def get_shap_for_single_row(pipe, input_df):
    ...
    # Infer numeric feature list from ColumnTransformer definition if available
    try:
        num_cols = preprocess.transformers_[0][2]
        numeric_features = list(num_cols)
    except Exception:
        numeric_features = [f"x{i}" for i in range(X_prep.shape[1])]
    ...
    try:
        ...
        if feature_names.shape[0] != X_prep.shape[1]:
            feature_names = np.array([f"x{i}" for i in range(X_prep.shape[1])], dtype=object)
    except Exception:
        feature_names = np.array([f"x{i}" for i in range(X_prep.shape[1])], dtype=object)
...
"""
        ...
        st.markdown("**Portfolio App:** (this site)")
...
- Clinical consistency rules (adult-only ages; no Visit 2 without Visit 1)
"""
    )
    ...
    with st.form("participant_form"):
        ...
        with c3:
            communication_score = st.slider("Communication Score (0–100)", 0.0, 100.0, 60.0)
            st.caption("Visits")
    ...
    st.write(f"Predicted class: {'Dropout (1)' if pred == 1 else 'Completer (0)'}")
